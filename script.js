...
let positionWatchId = null;          // For piping
let mapPositionWatchId = null;     // NY: For kartmarkør-oppdatering
let previousDistanceToTarget = null;
let finishMarker = null;

// === GLOBAL KONFIGURASJON ===
// ... (TOTAL_POSTS, POST_LOCATIONS, START_LOCATION, FINISH_LOCATION som i versjon #20) ...
const TOTAL_POSTS = 8; 
...
function startContinuousUserPositionUpdate() {
    ...
    if (mapPositionWatchId !== null) return; // Allerede i gang

    console.log("Starter kontinuerlig GPS posisjonssporing for kart og pip.");
    previousDistanceToTarget = null; 
    
    // Felles watchPosition for både kartoppdatering og piping
    mapPositionWatchId = navigator.geolocation.watchPosition(
        (position) => { // Success callback
            updateUserPositionOnMap(position); // Alltid oppdater kartmarkør
            if (isGpsAudioEnabled) { // Kun oppdater pip hvis lyd er på
                updateProximityBeeps(position);
            }
        }, 
        (error) => { // Error callback
            handleGeolocationError(error); 
            stopContinuousUserPositionUpdate(); // Stopp alt ved feil
            if(toggleGpsAudioButton) toggleGpsAudioButton.textContent = "🛰️ GPS Feil";
            isGpsAudioEnabled = false; // Skru av GPS-lyd ved feil
            localStorage.setItem('rebusGpsAudioEnabled', false);
        },
...
function stopContinuousUserPositionUpdate() {
    stopProximityBeepsInterval(); // Stopper pip-intervallet
    if (mapPositionWatchId !== null) { 
        navigator.geolocation.clearWatch(mapPositionWatchId); 
        mapPositionWatchId = null; 
        console.log("Stoppet kontinuerlig GPS sporing for kart og pip."); 
    }
    previousDistanceToTarget = null; 
    // Ikke fjern userPositionMarker her, den kan bli stående med siste kjente posisjon
}


document.addEventListener('DOMContentLoaded', () => {
    // ... (HTML element referanser og KONFIGURASJON som i versjon #20) ...
    ...
    function setupGpsAudioControls() {
        if (!toggleGpsAudioButton || !gpsAudioVolumeSlider) { /* ... (som før) ... */ console.warn("GPS lydkontroll-elementer mangler."); const gpsControlsDiv = document.getElementById('gps-audio-controls'); if(gpsControlsDiv) gpsControlsDiv.style.display = 'none'; return; }
        ...
        if (isGpsAudioEnabled && currentTeamData && currentTeamData.completedPostsCount < TOTAL_POSTS) { 
            startContinuousUserPositionUpdate(); // Bruker nå denne
        }
        ...
        toggleGpsAudioButton.addEventListener('click', () => {
            ...
            if (isGpsAudioEnabled && currentTeamData && currentTeamData.completedPostsCount < TOTAL_POSTS) { 
                ...
                stopContinuousUserPositionUpdate(); // Bruker nå denne
            }
        });
    ...
    // ... (setupMusicControls, updatePageText, showRebusPage, showTabContent, saveState, loadState, clearState, resetPageUI, resetAllPostUIs, initializeTeam, handlePostUnlock, handleTaskCheck, updateUIAfterLoad som i versjon #20)
    // FOR Å UNNGÅ EN EKSTREMT LANG FIL IGJEN, ER RESTEN AV KODEN HER DEN SAMME SOM I script.js versjon #20
    // DU MÅ ERSTATTE HELE FILEN MED DENNE NYE VERSJONEN.
    // De viktigste endringene er i `initMap`, `updateProximityBeeps`, `setupGpsAudioControls`, 
    // og introduksjonen av `updateUserPositionOnMap`, `startContinuousUserPositionUpdate`, `stopContinuousUserPositionUpdate`.
