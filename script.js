...
const POST_LOCATIONS = [ /* ... (som før) ... */ 
    { lat: 60.8007539616181, lng: 10.646227725129991, title: "Post 1", name: "Ved den røde husken"}, // Lagt til 'name' for beskrivelse
...
const FINISH_LOCATION = { lat: 60.80140295692265, lng: 10.643869988530302, title: "MÅL: Hovedinngang Kafe" }; // Mer beskrivende tittel

// === GOOGLE MAPS API CALLBACK ===
window.initMap = function() { /* ... (som i versjon #18, ingen store endringer her) ... */ 
    ...
    if (currentTeamData && currentTeamData.completedPostsCount < TOTAL_POSTS) {
        ...
        updateMapMarker(currentPostGlobalId, false); // false for ikke å indikere at det er målet
    } else if (currentTeamData && currentTeamData.completedPostsCount >= TOTAL_POSTS) { 
        updateMapMarker(null, true); // Vis mål når ferdig
    }
    showUserPosition(); 
...
function updateMapMarker(postGlobalId, isFinalTarget = false) { 
    ...
    let location;
    let markerTitle;
    let markerIconUrl = 'https://maps.google.com/mapfiles/ms/icons/red-dot.png'; // Standard for neste post

    if (isFinalTarget) {
        location = FINISH_LOCATION;
        markerTitle = FINISH_LOCATION.title;
        markerIconUrl = 'https://maps.google.com/mapfiles/ms/icons/blue-dot.png'; // Blå prikk for målet
    } else {
        if (postGlobalId < 1 || postGlobalId > POST_LOCATIONS.length) { 
            console.error("Ugyldig post ID for kartmarkør:", postGlobalId); return; 
        }
        location = POST_LOCATIONS[postGlobalId - 1];
        markerTitle = `Neste: ${location.title}`;
    }
...
// ... (clearMapMarker, handleGeolocationError, showUserPosition, GPS Audio funksjoner som i versjon #18) ...
...
function playBeep(frequency = 880, duration = 100, volume = gpsAudioVolume) { if (!audioContext || !isGpsAudioEnabled) return; const oscillator = audioContext.createOscillator(); const gainNode = audioContext.createGain(); oscillator.connect(gainNode); gainNode.connect(audioContext.destination); oscillator.type = 'sine'; oscillator.frequency.setValueAtTime(frequency, audioContext.currentTime); gainNode.gain.setValueAtTime(volume * 0.5, audioContext.currentTime); oscillator.start(); oscillator.stop(audioContext.currentTime + duration / 1000); }
...
function updateProximityBeeps(position) { if (!isGpsAudioEnabled || !currentTeamData || currentTeamData.completedPostsCount >= TOTAL_POSTS) { stopProximityBeeps(); return; } const userLat = position.coords.latitude; const userLng = position.coords.longitude; const nextPostIndexInSequence = currentTeamData.currentPostArrayIndex; if (nextPostIndexInSequence >= currentTeamData.postSequence.length) { stopProximityBeeps(); return; } const nextPostGlobalId = currentTeamData.postSequence[nextPostIndexInSequence]; if (nextPostGlobalId === undefined || nextPostGlobalId < 1 || nextPostGlobalId > POST_LOCATIONS.length) { stopProximityBeeps(); return; } const targetLocation = POST_LOCATIONS[nextPostGlobalId - 1]; const distance = calculateDistance(userLat, userLng, targetLocation.lat, targetLocation.lng); console.log(`Avstand til Post ${nextPostGlobalId}: ${distance.toFixed(0)}m`); let beepInterval = 0; let beepFrequency = 880; let beepDuration = 100; if (distance <= 5) { beepInterval = 300; beepFrequency = 1200; beepDuration = 80; } else if (distance <= 10) { beepInterval = 600; beepFrequency = 1000; } else if (distance <= 25) { beepInterval = 1000; } else if (distance <= 50) { beepInterval = 2000; } else if (distance <= 100) { beepInterval = 3500; } else if (distance > 100 ) { beepInterval = 7000; beepFrequency = 600; beepDuration = 150;} if (proximityBeepIntervalId) { clearInterval(proximityBeepIntervalId); proximityBeepIntervalId = null; } if (beepInterval > 0) { playBeep(beepFrequency, beepDuration, gpsAudioVolumeSlider.valueAsNumber); proximityBeepIntervalId = setInterval(() => { playBeep(beepFrequency, beepDuration, gpsAudioVolumeSlider.valueAsNumber); }, beepInterval); } }
function startProximityBeeps() { if (!navigator.geolocation) { console.warn("Geolocation ikke støttet."); isGpsAudioEnabled = false; if(toggleGpsAudioButton) toggleGpsAudioButton.textContent = "🛰️ GPS Av"; return; } if (positionWatchId !== null) return; initializeAudioContext(); console.log("Starter GPS posisjonssporing for pip."); positionWatchId = navigator.geolocation.watchPosition( updateProximityBeeps, (error) => { handleGeolocationError(error); stopProximityBeeps(); if(toggleGpsAudioButton) toggleGpsAudioButton.textContent = "🛰️ GPS Feil"; }, { enableHighAccuracy: true, maximumAge: 5000, timeout: 10000 } ); }
function stopProximityBeeps() { if (proximityBeepIntervalId) { clearInterval(proximityBeepIntervalId); proximityBeepIntervalId = null; } if (positionWatchId !== null) { navigator.geolocation.clearWatch(positionWatchId); positionWatchId = null; console.log("Stoppet GPS sporing."); } }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const TOTAL_POSTS = 8; // Allerede global, men kan være nyttig her også
    const TEAM_CONFIG = { /* ... (som før) ... */
    ...
    function updatePageText(pageElement, teamPostNumber, globalPostId) {
        ...
        const taskTitleElement = pageElement.querySelector('.post-task-title-placeholder'); // Beholder oppgavens unike tittel

        if (titleElement) {
            titleElement.textContent = `Lagets ${teamPostNumber}. Post: Finn Ankomstkoden! 🗝️`;
        }
        if (introElement) {
            const postDetails = POST_LOCATIONS[globalPostId -1];
            let postName = postDetails ? postDetails.name : `Post ${globalPostId}`;
            introElement.textContent = `Dere har nådd deres ${teamPostNumber}. post i rebusløpet, som er ved ${postName}. Se dere rundt og finn ankomstkoden for å låse opp oppgaven!`;
            if (teamPostNumber === TOTAL_POSTS) { // Siste post
                 titleElement.textContent = `Lagets Siste Post: Finn Ankomstkoden! 🏁`;
                 introElement.textContent = `Dette er deres siste post før det store målet! Finn ankomstkoden ved ${postName} for å løse den siste oppgaven.`;
            }
        }
        // Oppgavetittel (f.eks. "Gåtefull Start!") beholdes som den er i HTML, da den er unik for den globale posten.
    }
    ...
    // ... (resten av funksjonene: showTabContent, saveState, loadState, clearState, resetPageUI, resetAllPostUIs, initializeTeam, handlePostUnlock, handleTaskCheck, updateUIAfterLoad, setupMusicControls, setupGpsAudioControls)
    // ... (alle event listeners og den avsluttende initialiseringen)
    // FOR Å UNNGÅ EN EKSTREMT LANG FIL IGJEN, ER RESTEN AV KODEN HER DEN SAMME SOM I script.js versjon #18
    // DU MÅ ERSTATTE HELE FILEN MED DENNE NYE VERSJONEN, INKLUDERT DENNE KOMMENTAREN OG DET SOM FØLGER.
    // DEN ENESTE STORE ENDRINGEN ER updatePageText og kallet til den i showRebusPage.
    // Samt endringer i updateMapMarker og initMap for mål-markøren.
    ...
    function resetAllPostUIs() {
        for (let i = 1; i <= TOTAL_POSTS; i++) {
            ...
            if(taskFeedback) { taskFeedback.textContent = ''; taskFeedback.className = 'feedback'; }
            // Gjenopprett generiske titler ved full reset
            const titlePlaceholder = document.querySelector(`#post-${i}-page .post-title-placeholder`);
            if(titlePlaceholder) titlePlaceholder.textContent = "Neste Post: Finn Ankomstkoden! 🗝️";
            const introPlaceholder = document.querySelector(`#post-${i}-page .post-intro-placeholder`);
    ...
    function initializeTeam(teamCode) {
        ...
        if (config) {
            ...
            console.log(`Team ${currentTeamData.name} startet! Deres 1. post (globalt: ${firstPostInSequence})`);
        } else {
        ...
        if (isCorrectMain || isCorrectAlternative || isFasit) {
            ...
            if (currentTeamData.completedPostsCount < TOTAL_POSTS) {
                if (currentTeamData.currentPostArrayIndex < currentTeamData.postSequence.length) {
                    ...
                    setTimeout(() => { showRebusPage('finale-page'); updateMapMarker(null, true); stopProximityBeeps(); }, 1200); // Vis mål
                }
            } else { setTimeout(() => { showRebusPage('finale-page'); updateMapMarker(null, true); stopProximityBeeps(); }, 1200); } // Vis mål
        } else { 
    ...
    function setupMusicControls() { /* ... (som før) ... */ 
        if (!backgroundAudio || !playPauseButton || !muteUnmuteButton || !volumeSlider) { console.warn("Musikk-kontroll elementer mangler."); if(document.getElementById('music-controls')) document.getElementById('music-controls').style.display = 'none'; return; }
        const savedVolume = localStorage.getItem('rebusMusicVolume'); const savedMuted = localStorage.getItem('rebusMusicMuted') === 'true';
        if (savedVolume !== null) { backgroundAudio.volume = parseFloat(savedVolume); volumeSlider.value = parseFloat(savedVolume); } else { backgroundAudio.volume = 0.5; volumeSlider.value = 0.5; }
        ...
        backgroundAudio.load(); backgroundAudio.addEventListener('canplaythrough', () => { console.log("Musikk kan spilles."); if (backgroundAudio.paused) { backgroundAudio.play().then(() => { if (playPauseButton) playPauseButton.textContent = '⏸️'; }).catch(e => { console.warn('Autoplay forhindret:', e.name, e.message); if (playPauseButton) playPauseButton.textContent = '▶️'; }); } });
    ...
    function setupGpsAudioControls() { /* ... (som i versjon #18, med sjekk for currentTeamData) ... */ 
        if (!toggleGpsAudioButton || !gpsAudioVolumeSlider) { console.warn("GPS lydkontroll-elementer mangler."); if(document.getElementById('gps-audio-controls')) document.getElementById('gps-audio-controls').style.display = 'none'; return; }
        const savedGpsAudioEnabled = localStorage.getItem('rebusGpsAudioEnabled') === 'true'; const savedGpsAudioVolume = localStorage.getItem('rebusGpsAudioVolume');
        isGpsAudioEnabled = savedGpsAudioEnabled;
        if (savedGpsAudioVolume !== null) { gpsAudioVolume = parseFloat(savedGpsAudioVolume); gpsAudioVolumeSlider.value = gpsAudioVolume; } else { gpsAudioVolumeSlider.value = gpsAudioVolume; }
        ...
        toggleGpsAudioButton.addEventListener('click', () => { isGpsAudioEnabled = !isGpsAudioEnabled; toggleGpsAudioButton.textContent = isGpsAudioEnabled ? "🛰️ GPS På" : "🛰️ GPS Av"; localStorage.setItem('rebusGpsAudioEnabled', isGpsAudioEnabled); if (isGpsAudioEnabled && currentTeamData && currentTeamData.completedPostsCount < TOTAL_POSTS) { startProximityBeeps(); } else { stopProximityBeeps(); } });
    ...
    if (loadState()) { 
        ...
        if (currentTeamData && currentTeamData.completedPostsCount >= TOTAL_POSTS) {
            ...
            if (map) updateMapMarker(null, true); // Vis mål på kartet hvis ferdig
