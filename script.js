...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function startProximityBeeps() {
        ...
        positionWatchId = navigator.geolocation.watchPosition(
            ...
            (error) => {
                handleGeolocationError(error); stopProximityBeeps(); 
    ...
    function initializeTeam(teamCode) {
        ...
        if (config) {
            ...
            showRebusPage(`post-${firstPostInSequence}-page`); updateMapMarker(firstPostInSequence); showUserPosition(); 
        ...
        if (isCorrectMain || isCorrectAlternative || isFasit) {
            ...
            if (currentTeamData.completedPostsCount < TOTAL_POSTS) {
                if (currentTeamData.currentPostArrayIndex < currentTeamData.postSequence.length) {
                    ...
                    setTimeout(() => { showRebusPage(`post-${nextPostGlobalId}-page`); updateMapMarker(nextPostGlobalId); }, 1200);
                } else { 
    ...
    // === INITIALIZATION ===
    setupMusicControls(); // Denne SKAL være her
