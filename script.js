...
let previousDistanceToTarget = null; // NY: For å lagre forrige avstand

// === KONFIGURASJON FOR KART (Globalt tilgjengelig for initMap) ===
// ... (POST_LOCATIONS, START_LOCATION, FINISH_LOCATION som i versjon #19) ...
const POST_LOCATIONS = [ { lat: 60.8007539616181, lng: 10.646227725129991, title: "Post 1", name: "Ved den røde husken"}, { lat: 60.80017468739349, lng: 10.64510651928592, title: "Post 2", name: "Ved den store eika"}, { lat: 60.80072782302861, lng: 10.644889579638045, title: "Post 3", name: "På den lengste benken"}, { lat: 60.80048329479234, lng: 10.643492818098643, title: "Post 4", name: "Ved informasjonstavlen"}, { lat: 60.80045228531585, lng: 10.642988549931982, title: "Post 5", name: "Ved flaggstangen"}, { lat: 60.7998031467142, lng: 10.643149576741504, title: "Post 6", name: "Ved sykkelstativet"}, { lat: 60.7990979034987, lng: 10.64366234869697, title: "Post 7", name: "Ved steinmuren"}, { lat: 60.79974498905187, lng: 10.64269195029222, title: "Post 8", name: "Ved hovedinngangen til området"} ];
...
function playBeep(frequency = 880, duration = 100, volume = gpsAudioVolume, type = 'sine') { // type kan være 'sine', 'square', 'sawtooth', 'triangle'
...
function updateProximityBeeps(position) {
    if (!isGpsAudioEnabled || !currentTeamData || currentTeamData.completedPostsCount >= TOTAL_POSTS) {
        stopProximityBeeps();
        previousDistanceToTarget = null; // Nullstill forrige avstand
        return;
    }
    ...
    let beepInterval = 0;
    let baseBeepFrequency = 880; // Standard A5
    let beepDuration = 100;  // ms
    let beepType = 'sine'; // Standard pipelyd

    // Bestem intervall basert på avstand
    ...
    // Juster frekvens basert på retning (hvis vi har en forrige avstand å sammenligne med)
    if (previousDistanceToTarget !== null) {
        if (currentDistance < previousDistanceToTarget - 0.5) { // Nærmere (minus 0.5m for å unngå støy fra små GPS-variasjoner)
            finalBeepFrequency = baseBeepFrequency * 1.2; // Lysere tone
            beepType = 'sawtooth'; // Litt skarpere lyd
            console.log("Nærmere!");
        } else if (currentDistance > previousDistanceToTarget + 0.5) { // Lenger unna (pluss 0.5m)
            finalBeepFrequency = baseBeepFrequency * 0.8; // Dypere tone
            beepType = 'square'; // Annen klang
            console.log("Lenger unna!");
        }
        // Hvis omtrent samme avstand, behold baseBeepFrequency og beepType = 'sine'
    }

    if (proximityBeepIntervalId) { clearInterval(proximityBeepIntervalId); proximityBeepIntervalId = null; }
    
    if (beepInterval > 0) {
        playBeep(finalBeepFrequency, beepDuration, gpsAudioVolumeSlider.valueAsNumber, beepType);
        proximityBeepIntervalId = setInterval(() => {
            // For neste pip i intervallet, bruk base frekvens med mindre vi implementerer kontinuerlig retningssjekk
            // For enkelhets skyld, la oss bare bruke den sist beregnede retningsindikerte frekvensen for dette intervallet.
            // En mer avansert løsning ville re-evaluert retning oftere enn beepInterval.
            playBeep(finalBeepFrequency, beepDuration, gpsAudioVolumeSlider.valueAsNumber, beepType);
        }, beepInterval);
    }
...
function startProximityBeeps() { /* ... (som i versjon #17) ... */ 
    ...
    previousDistanceToTarget = null; // Nullstill når sporing starter
    positionWatchId = navigator.geolocation.watchPosition(
        ...
        { enableHighAccuracy: true, maximumAge: 3000, timeout: 7000 } // Justert maximumAge og timeout
    );
}
function stopProximityBeeps() { /* ... (som i versjon #17) ... */ 
    ...
    previousDistanceToTarget = null; // Nullstill
}
// ... (Resten av script.js fra Versjon #18, inkludert setupGpsAudioControls, setupMusicControls, og all kjerne- og initialiseringslogikk) ...
// FOR Å UNNGÅ EN EKSTREMT LANG FIL IGJEN, ER RESTEN AV KODEN HER DEN SAMME SOM I script.js versjon #18
// DU MÅ ERSTATTE HELE FILEN MED DENNE NYE VERSJONEN.
// De viktigste endringene er i `updateProximityBeeps`, `playBeep`, `startProximityBeeps`, `stopProximityBeeps`
// og introduksjonen av `previousDistanceToTarget`.
    ...
    function showRebusPage(pageId) { pages.forEach(page => page.classList.remove('visible')); const nextPage = document.getElementById(pageId); if (nextPage) { nextPage.classList.add('visible'); const container = document.querySelector('.container'); if (container) window.scrollTo({ top: container.offsetTop - 20, behavior: 'smooth' }); resetPageUI(pageId); } else { console.error("Side ikke funnet:", pageId); clearState(); showRebusPage('intro-page'); } }
