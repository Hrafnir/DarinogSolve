...
function playBeep(frequency = 880, duration = 100, volume = gpsAudioVolume, type = 'sine') { 
    ...
    gainNode.gain.setValueAtTime(actualVolume * 0.5, audioContext.currentTime); 
...
function updateUserPositionOnMap(position) {
    ...
    if (userPositionMarker) {
        ...
        userPositionMarker = new google.maps.Marker({
            ...
            icon: { path: google.maps.SymbolPath.CIRCLE, scale: 7, fillColor: "#1976D2", fillOpacity: 1, strokeWeight: 2, strokeColor: "white" }
        });
    }
}
...
function updateProximityBeepsLogic(position) { // Selve logikken for piping
    ...
    if (nextPostIndexInSequence >= currentTeamData.postSequence.length) { stopProximityBeepsInterval(); previousDistanceToTarget = null; return; }
